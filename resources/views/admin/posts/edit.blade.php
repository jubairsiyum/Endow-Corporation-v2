...
<form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data" id="post-form">
    ...
    <div class="panel" style="padding:28px;">
        <div style="display:flex;flex-direction:column;gap:24px;">
            ...
            {{-- Title --}}
            <x-admin.input name="title" label="Post Title" :value="$post->title" required />

            {{-- Slug + Author Row --}}
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <x-admin.input name="slug" label="Slug" :value="$post->slug" />
                <x-admin.input name="author" label="Author Name" :value="$post->author" />
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                ...
                <x-admin.input name="category" label="Category (Text override)" :value="$post->category" />
            </div>
            ...
            {{-- SEO Fields --}}
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <x-admin.input name="meta_title" label="Meta Title (SEO)" :value="$post->meta_title" />
                <x-admin.input name="og_image" label="OG Image URL" :value="$post->og_image" />
            </div>
            <div>
            ...
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
                ...
                <input type="checkbox" name="is_published" value="1" {{ old('is_published', $post->is_published) ? 'checked' : '' }} style="width:15px;height:15px;border-radius:4px;accent-color:#EF4444;cursor:pointer;">
            ...
            <div style="display:flex;align-items:center;gap:12px;padding-top:8px;border-top:1px solid var(--border);">
                <button type="submit" class="btn btn-primary">
                    <i class="fa-solid fa-check text-xs"></i> Update Post
                </button>
